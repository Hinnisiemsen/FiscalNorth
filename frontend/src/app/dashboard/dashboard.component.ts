...
export class DashboardComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        forkJoin({
            ...
            next: ({ user, accounts, contracts, currentInsights, previousInsights, budgets, notifications }) => {
                ...
                this.monthlyTrends = [
                    ...currentInsights.monthlyTrends,
                    ...previousInsights.monthlyTrends,
                ];
                ...
                const expenseChange = percentChange(this.currentTotals.expenses, this.previousTotals.expenses);
        ...
        this.ai.chat(query).subscribe({
            ...
            error: (err) => {
                this.panelAiState[panel] = {
                    ...
                    error:
                        err?.error?.message ||
                        this.language.instant('dashboard.errors.analysisFailed'),
                    text: '',
                };
            },
        });
    ...
    verlaufTeaser(): string {
        ...
        if (change > 0) {
            return this.language.instant('dashboard.teaserTrendMore', { pct: Math.abs(change).toFixed(0) });
        }
    ...
    budgetsTeaser(): string {
        ...
        if (pct >= 80) {
            return this.language.instant('dashboard.teaserBudgetAt', { name: worst.name, pct: pct.toFixed(0) });
        }
